...
export default {
	data() {
		return {
			isCollect: false,
			exportsList: baseUrl + '/wechatmanage/wechatExport?type=2&token='+localStorage.getItem('token'),
			...
			searchItem: {
				...
				collectionstate: '1',//收藏状态1 收藏 2 未收藏
				statusVal: '', //选中的状态
				deliverystate: '',//投放状态
				currentPage1: 1,  //当前页
				pageSize: 10, //分页数
	...
	methods: {
		...
		async getWXKInf() {
			let obj = {
				collectionstate: this.searchItem.collectionstate, //收藏状态1 收藏 2 未收藏
		...
		async cancelAreen(row) {
			let cancelAreen = await wxkcancelAreen({ id: row.houseid });
