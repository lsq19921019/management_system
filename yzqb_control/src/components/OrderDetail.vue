      ...
      <el-row>
        ...
        <el-table :data="orderHandleList" v-loading="orderHandleListLoading">
          ...
          <el-table-column prop="offineRefundType" label="还款渠道">
            <template slot-scope="scope">{{formatChannel(scope.row.offineRefundType)}}</template>
          </el-table-column>
    ...
    <template>
      <el-row>
        ...
        <el-col :span="12" class="grid-content">
          ...
          <!-- <div><span>婚姻状况:</span><span>{{ getMarriage(idAddressInfo.marriage)}} </span></div> -->
...
<!--      <el-table-column prop="amountApply" label="用户名">
          ...
          <el-table-column prop="" label="姓名">
            <template slot-scope="scope">{{refundName}}</template>
          </el-table-column>
          <el-table-column prop="" label="操作状态">
              <template slot-scope="scope">拒绝</template>
          </el-table-column>
        </el-table>
      </el-row>
      <div class="divid-line"></div>
    ...
    <template>
      <el-row>
        <h1 class="one-title">订单审核&nbsp;<el-button type="primary" size="small" @click="toExamine(3)" v-if="orderDetail.status == 13">审核通过</el-button>&nbsp;<el-button type="primary" size="small" @click="toExamine(10)" v-if="orderDetail.status == 13">审核失败</el-button></h1>
 ...
 ManualSetOrderDialogVisible:false,
	...
	/**
      ...
      historyDetail(row){
        ...
        //   uuid:row.uuid||''
        // }});
        // :seen="item.seen" :uuid="item.uuid" :userUuid="item.userUuid" 
        this.$emit('getOrderDetail',{seen:true,uuid:row.uuid,userUuid:row.userUuid,});
    ...
    mounted: function () {
      console.log(this.seen+'====='+this.userUuid+'====='+this.uuid);
