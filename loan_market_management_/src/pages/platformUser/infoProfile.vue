...
<template>
    <div class="_contain">
        <div class="total">
            注册人数：<span>{{ otherData.registerTotal }}</span>
            实名人数：<span>{{ otherData.authTotal }}</span>
        </div>
        <!-- 条件查询组件-->
        <search-component :pubConfig="pubConfig" :subConfig="searchConfig" @search="getList" v-if="isShowSearchComponent"></search-component>
        
        <!-- 表格&分页 -->
        <table-component :tableConfig="tableConfig" :pubConfig="pubConfig" ref="myTable"></table-component>
    </div>
</template>
