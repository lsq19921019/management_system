  ...
  <section>
    ...
    <el-form :inline="true" :model="searchForm" class="toolbar" label-position="right" label-width="70px">
      <el-row>
        ...
        <el-form-item label="手动扣款状态">
          <el-select v-model="searchForm.cutPaymentStatus" placeholder="请选择" clearable>
            <el-option v-for="item in cutStatusSelection" :label="item.name" :key="item.code" :value="item.code"></el-option>
          </el-select>
        </el-form-item>
    ...
    <template>
      <el-table :data="gridData" highlight-current-row v-loading="gridLoading" class="grid" @row-click="rowClick">
        ...
        <el-table-column label="手动扣款状态" prop="orderStep">
          <template scope="scope">
            <span>{{getCutStatus(scope.row.cutPaymentStatus)}}</span>
          </template>
        </el-table-column>
...
<script>
  ...
  import orderDetailpage from '../../components/SecondBuckleOrderDetail'
  export default {
    data () {
      return {
        ...
        qudaolist: [],
        cutStatusSelection:enums.cutStatusSelection,
    ...
    methods: {
      ...
      getCutStatus(step){
        return enums.getCutStatus(step);
      },
      ...
      check(row){
        window.open('/#/SecondBuckleOrderDetail?seen=true&userUuid='+row.userUuid+
          '&uuid='+row.uuid);
        //   alert(111);
      },
      rowClick(row){
      ...
      bindGrid(searchType) {
        ...
        if(this.searchForm.value4&&this.searchForm.value4.length>0&&this.searchForm.value4[0]){
          ...
          this.searchForm.createTimeTimeEnd = _v.getFullYear()+'-'+_m+'-'+_v.getDate();
        ...
        this.$http.post('/manage/secondBuckleOrderList', _data).then(response => {
          if (1 == response.body.code) {
