    ...
    var mainFn = {
        ...
        getSmsCode: function (type, cellphone, imgCode, imei) {
            ...
            tools.fetchAjax(apiUrl.data + '/message/sms_code', reqData, 'post', {imei: imei}).done(function (ret) {
                if (ret.status === 0 || ret.status === '0') {
                    ...
                    mainFn.getImgCode();
                    // imgCode.addClass('border-tip').focus();
                    tools.tipsAlert(ret.msg);  
                }
            });
        },
        ...
        bindbank: function () {
            ...
            $bank.find('.get-sms-code').on('click', function () {
                ...
                if (!imgCode) {
                    $bank.find('.code-input').addClass('border-tip').focus();
                    $bank.find( $bank.find('.code-input')).siblings('i').text('请输入图形验证码').removeClass('hide');
                    return mainFn.hideTip($bank),!1;
                } else {
                    $bank.find('.code-input').removeClass('border-tip');
            ...
            $bank.find('.con-btn').on('click',function(){
                var imei = $bank.find('.img-code').attr('data-imei'),_that=$(this);
                if(_that.hasClass('disabled')){return false}
                var bankReg = {
                    chineseName:/^([a-zA-Z\u4e00-\u9fa5]){2,12}$/,
                    bankAccount :/^([1-9]{1})(\d{14}|\d{18})$/,
                    yzmReg:/^([0-9]){4}$/,
                    PhonCodeReg:/^([0-9]){6}$/
                }
                var doms = {
                    userName:$bank.find('.name'),
                    bankName:$bank.find('.khyh'),
                    branchAddress:$bank.find('.zhmc'),
                    cardNo:$bank.find('.yhkh'),
                    code_:$bank.find('.sjyzm'),
                    province:'',                   
                    city:'',
                    phone:getPhone
                }
                ...
                if(!reqData.userName){
                    $bank.find(doms.userName).siblings('i').text('用户名不能为空').removeClass('hide');
                    doms.userName.addClass('border-tip').focus();
                    return mainFn.hideTip($bank),!1;

                }else if(!bankReg.chineseName.test($.trim(reqData.userName))){
                    $bank.find(doms.userName).siblings('i').text('用户名格式错误，长度为2-12位').removeClass('hide');
                    doms.userName.addClass('border-tip').focus();
                    return mainFn.hideTip($bank),!1;
                }else{
                    doms.userName.removeClass('border-tip');
                    $bank.find(doms.userName).siblings('i').text('').addClass('hide');
                }

               if(reqData.bankName=='请选择'){
                    $bank.find(doms.bankName).siblings('i').text('请选择开户银行').removeClass('hide');
                    return mainFn.hideTip($bank),!1;
                }else{
                    $bank.find(doms.bankName).siblings('i').text('').addClass('hide');
                }

                if(!reqData.branchAddress){
                    $bank.find(doms.branchAddress).siblings('i').text('支行名称不能为空').removeClass('hide');
                    doms.branchAddress.addClass('border-tip').focus();
                    return mainFn.hideTip($bank),!1;
                }else{
                    doms.branchAddress.removeClass('border-tip');
                    $bank.find(doms.branchAddress).siblings('i').text('').addClass('hide');
                }

                if(!reqData.cardNo){
                    $bank.find(doms.cardNo).siblings('i').text('请输入银行卡号').removeClass('hide');
                    doms.cardNo.addClass('border-tip').focus();
                    return mainFn.hideTip($bank),!1;
                }else if(!bankReg.bankAccount.test($.trim(reqData.cardNo))){
                    $bank.find(doms.cardNo).siblings('i').text('银行卡号格式错误').removeClass('hide');
                    doms.cardNo.addClass('border-tip').focus();
                    return mainFn.hideTip($bank),!1;
                }else{
                    doms.cardNo.removeClass('border-tip');
                    $bank.find(doms.cardNo).siblings('i').text('').addClass('hide');
                }
                ...
                if(!reqData.cardNo){
                 ...
                 if($bank.find('.code-input').val() == ''){
                    $bank.find('.code-input').addClass('border-tip').focus();
                    $bank.find( $bank.find('.code-input')).siblings('i').text('请输入图形验证码').removeClass('hide');
                    return mainFn.hideTip($bank),!1;
                }else if(!bankReg.yzmReg.test($.trim($bank.find('.code-input').val()))){
                    $bank.find('.code-input').addClass('border-tip').focus();
                    $bank.find( $bank.find('.code-input')).siblings('i').text('图形验证码格式错误').removeClass('hide');
                    return mainFn.hideTip($bank),!1;
                }else{
                    $bank.find('.code-input').removeClass('border-tip');
                    $bank.find($bank.find('.code-input')).siblings('i').text('').addClass('hide');
                }

                if(!reqData.code){
                    $bank.find(doms.code_).addClass('border-tip').focus();
                    $bank.find(doms.code_).siblings('i').text('手机验证码不能为空').removeClass('hide');
                    return mainFn.hideTip($bank),!1;
                }else if(!bankReg.PhonCodeReg.test($.trim(reqData.code))){
                    $bank.find(doms.code_).addClass('border-tip').focus();
                    $bank.find(doms.code_).siblings('i').text('手机验证码格式错误').removeClass('hide');
                    return mainFn.hideTip($bank),!1;
                }else{
                    $bank.find(doms.code_).removeClass('border-tip');
                    $bank.find(doms.code_).siblings('i').text('').addClass('hide');
                }
        ...
        // 安全认证页面
        safePage: function () {
            if(location.search.indexOf("?") >= 0 && location.search.substring(1) == 'first'){
                $("#title-set").text("设置交易密码")
            }
            ...
            $safe.find('.update-password').on('click', 'a.confirm', function () {
                ...
                } else if(!phoneReg.test($.trim(oldPassword))){
                    tools.tipsAlert('原密码格式错误');
                }else if (!newPassword ) {
                    tools.tipsAlert('请输入您的新密码');
                }else if(!phoneReg.test($.trim(newPassword))){
                    tools.tipsAlert('密码长度为6-20个字符[区分大小写，支持英文数字]');
                }else if(!confirmPassword){
                    tools.tipsAlert('请确认新密码');
            ...
            //修改交易密码
            var $updatepass=$(".update-pay-passwords");
            ...
            $updatepass.find('.get-sms-code').on('click', function () {
                ...
                // }
                 if (!imgCode) {
                    $updatepass.find('.code-input').addClass('border-tip').focus();
                    $updatepass.find( $updatepass.find('.code-input')).siblings('i').text('请输入图形验证码').removeClass('hide');
                    return mainFn.hideTip($updatepass),!1;
                } else {
                    $updatepass.find('.code-input').removeClass('border-tip');
                    mainFn.getSmsCode(3, phone, imgCode, imei);
                }
            ...
            $updatepass.find(".confirm").on("click",function(){
                var doms = {
                    domYzm:$updatepass.find(".code-input"),
                    domPhoneyzm:$updatepass.find(".phoneyzm"),
                    domPassword:$updatepass.find(".new-password"),
                    domPasswords:$updatepass.find(".password-again"),
                };
                var regObj = {
                    yzmReg:/^([0-9]){4}$/,
                    PhonCodeReg:/^([0-9]){6}$/,
                    Pasreg : /^([a-zA-Z0-9_]){6,20}$/,
                };
                var yzm = $updatepass.find('.code-input').val() || '';
                    phoneyzm = $updatepass.find('.phoneyzm').val() || '',
                    password = $updatepass.find('.new-password').val() || '',
                    passwords = $updatepass.find('.password-again').val() || '';
                    passData={
                    code:phoneyzm,
                    value: DES3.encrypt('b8a5483ed47744f7b4978104afdcc075', JSON.stringify({
                        password: password,
                ...
                if(!yzm){
                    $updatepass.find(doms.domYzm).siblings('i').text('图片验证码不能为空').removeClass('hide');
                    doms.domYzm.addClass('border-tip').focus();
                    return mainFn.hideTip($updatepass),!1;
                }else if(!regObj.yzmReg.test($.trim(yzm))){
                    $updatepass.find(doms.domYzm).siblings('i').text('验证码格式错误').removeClass('hide');
                    doms.domYzm.addClass('border-tip').focus();
                    return mainFn.hideTip($updatepass),!1;
                }else{
                    doms.domYzm.removeClass('border-tip');
                    $updatepass.find(doms.domYzm).siblings('i').text('').addClass('hide');
                }
                if(!phoneyzm){
                    doms.domPhoneyzm.addClass('border-tip').focus();
                    $updatepass.find(doms.domPhoneyzm).siblings('i').text('手机验证码不能为空').removeClass('hide');
                    return mainFn.hideTip($updatepass),!1;
                }else if(!regObj.PhonCodeReg.test($.trim(phoneyzm))){
                    doms.domPhoneyzm.addClass('border-tip').focus();
                    $updatepass.find(doms.domPhoneyzm).siblings('i').text('手机验证码格式错误').removeClass('hide');
                    return mainFn.hideTip($updatepass),!1;
                }else{
                    doms.domPhoneyzm.removeClass('border-tip');
                    $updatepass.find(doms.domPhoneyzm).siblings('i').text('').addClass('hide');
                }
                if(!password){
                    doms.domPassword.addClass('border-tip').focus();
                    $updatepass.find(doms.domPassword).siblings('i').text('密码不能为空').removeClass('hide');
                    return mainFn.hideTip($updatepass),!1;
                }else if(!regObj.Pasreg.test($.trim(password))){
                    doms.domPassword.addClass('border-tip').focus();
                    $updatepass.find(doms.domPassword).siblings('i').text('密码格式错误').removeClass('hide');
                    return mainFn.hideTip($updatepass),!1;
                }else{
                    doms.domPassword.removeClass('border-tip');
                    $updatepass.find(doms.domPassword).siblings('i').text('').addClass('hide');
                }
                if(!passwords){
                    doms.domPasswords.addClass('border-tip').focus();
                    $updatepass.find(doms.domPasswords).siblings('i').text('请确认密码').removeClass('hide');
                    return mainFn.hideTip($updatepass),!1;
                }else if(password!=passwords){
                    doms.domPasswords.addClass('border-tip').focus();
                    $updatepass.find(doms.domPasswords).siblings('i').text('密码不一致,重新输入').removeClass('hide');
                    return mainFn.hideTip($updatepass),!1;
                }else{
                    doms.domPasswords.removeClass('border-tip');
                    $updatepass.find(doms.domPasswords).siblings('i').text('').addClass('hide');
                }
        ...
        hideTip:function(obj){
            clearTimeout(mainFn.setT)
            mainFn.setT= setTimeout(function(){
