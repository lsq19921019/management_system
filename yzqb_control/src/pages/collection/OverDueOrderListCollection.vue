...
<template>
      ...
      <el-row>
        ...
        <el-form-item label="性别">
          <el-select v-model="searchForm.sex" placeholder="请选择" clearable>
            <el-option v-for="item in sexLists" :label="item.name" :key="item.code" :value="item.code"></el-option>
          </el-select>
        </el-form-item>
  ...
  <!--         <el-table-column label="手机号" prop="mobileNumber" width="130">
          ...
          <el-table-column label="渠道" prop="channel" min-width="60">
            <template slot-scope="scope">{{getChannelName(scope.row.channel)}}</template>
          </el-table-column>
      </el-table>
    </template>
